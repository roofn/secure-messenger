...
func parseConfig() serverConfig {
	certPath := flag.String("cert", "/path/to/cert/server.pem", "Path to the server TLS certificate")
	keyPath := flag.String("key", "/path/to/cert/server.key", "Path to the server TLS private key")
	clientCAPath := flag.String("client-ca", "/path/to/cert/client_ca.pem", "Path to the client CA bundle")
